<script setup>
import { JsonSchemaConstant, RankerConstant, ResourceConstant, StateConstant } from '~/constants';
import { RankerActionExecutionPayloadFactory } from '~/models/server/ranker';
import { RankerDefaultOutput, RankerNode, RankerNodeData, RankerParameters, RankerPayload, RankerStateDefinition } from '~/models/workflow/state/task/ranker';
...
// Restore the form data
{
  ...
  state.payload = new RankerPayload(payload);
...
const handleUseExternalMemoryOutputChange = (v) => {
  state.formData.data.stateDefinition.inputOutput.resultSelector = v
    ...
    : StateConstant.ActionType.RANKER.defaultInputOutput.resultSelector;
  if (!v) {
...
const handleAbortOnErrorChange = (v) => {
  state.payload.defaultOutput = v ? null : new RankerDefaultOutput().removeActionType();
  update();
};
...
const update = async () => {
  ...
  state.formData.data = new RankerNodeData({
    stateDefinition,
...
<template>
  <WorkflowStateForm
    ref="form"
    :form-title-icon="StateConstant.ActionType.RANKER.icon"
    :form-title="$t('__workflowAction', { action: $t(StateConstant.ActionType.RANKER.i18nTitle) })"
  ...
  >
    <template #config-fields>
      ...
      />
      <template v-if="!jsonPathUtils.isJsonPath(state.rankerResource) && objUtils.isObject(state.rankerResource)">
        <template v-if="state.payload.ranker.rankerType">
        ...
        <ReferencePathInputGroup
          v-model="state.payload.ranker.docs"
          :default-value="RankerConstant.ActionExecutionParams.RANKER_DOCS.default"
          :expected-value-types="[JsonSchemaConstant.DataType.ARRAY.value]"
          :label="$t('__fieldDocument', 2)"
          :on-update="update"
          required
          enable-json-switch
        >
          <template #default="{ label }">
            <AppJsonSchemaRendererInput
              v-model:form-data="state.payload.ranker.docs"
              :label="label"
              :schema="RankerConstant.ActionExecutionParams.RANKER_DOCS.jsonSchema"
              :on-update="update"
            />
          </template>
        </ReferencePathInputGroup>
              ...
              <template #default="{ id, label }">
                <AppTextField
                  ...
                  @update:model-value="(v) => {
                    if (!v) {
                      state.payload.ranker.limit = null;
                    }
                    update();
                  }"
                />
        ...
        <AppFormFieldExpansionPanel :title="$t('__titleExecutionSettings')">
          ...
          <template v-if="!state.abortOnError">
            <ReferencePathInputGroup
              ...
              :default-value="new RankerDefaultOutput().removeActionType()"
              ...
              :schema="RankerConstant.ActionExecutionParams.DEFAULT_OUTPUT.jsonSchema"
              :on-update="update"
              enable-json-switch
            >
              <template #default="{ label }">
