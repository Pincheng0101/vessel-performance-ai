<script setup>
import { KnowledgeBaseConstant, ListConstant, LoaderConstant, ResourceConstant, RetrieverConstant } from '~/constants';
...
const parserMode = defineModel('parserMode', {
  type: String,
});
...
<template>
  ...
  />
  <AppInputGroup
    v-if="!props.hiddenFields.includes('parserMode')"
    v-slot="{ id, label }"
    :label="$t('__fieldParserMode')"
    :tooltip="$t('__tooltipUnsupervisedLoaderParserMode')"
  >
    <AppSelect
      :id="id"
      v-model="parserMode"
      :items="Object.values(LoaderConstant.ParserMode).map(item => ({
        ...item,
  ...
  <AppInputGroup
    ...
    required
  >
    <ResourceLoaderFormFieldsUnsupervisedSourceTable
      v-model="sources"
      :aria-label="label"
