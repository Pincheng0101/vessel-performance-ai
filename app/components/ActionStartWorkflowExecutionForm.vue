<script setup>
import { ActionExecutionConstant, JsonSchemaConstant, ResourceConstant, StartWorkflowExecutionConstant, StateConstant } from '~/constants';
import { Workflow } from '~/models/server/workflow';
import { StartWorkflowExecutionDefaultOutput, StartWorkflowExecutionNode, StartWorkflowExecutionNodeData, StartWorkflowExecutionParameters, StartWorkflowExecutionPayload, StartWorkflowExecutionStateDefinition } from '~/models/workflow/state/task/startWorkflowExecution';

const props = defineProps({
  node: {
    type: StartWorkflowExecutionNode,
    required: true,
  },
...
// Restore the form data
{
  ...
  state.payload = new StartWorkflowExecutionPayload(payload);
...
const handleUseExternalMemoryOutputChange = (v) => {
  state.formData.data.stateDefinition.inputOutput.resultSelector = v
    ...
    : StateConstant.ActionType.START_WORKFLOW_EXECUTION.defaultInputOutput.resultSelector;
  if (!v) {
...
const handleAbortOnErrorChange = (v) => {
  state.payload.defaultOutput = v ? null : new StartWorkflowExecutionDefaultOutput().removeActionType();
  update();
};
...
const update = async () => {
  const stateDefinition = new StartWorkflowExecutionStateDefinition({
    ...state.formData.data.stateDefinition,
    parameters: new StartWorkflowExecutionParameters({
      ...state.formData.data.stateDefinition.parameters,
      payload: new StartWorkflowExecutionPayload({
        ...state.payload,
      }),
    }),
  });

  state.formData.data = new StartWorkflowExecutionNodeData({
    stateDefinition,
...
<template>
  <WorkflowStateForm
    ref="form"
    :form-title-icon="StateConstant.ActionType.START_WORKFLOW_EXECUTION.icon"
    :form-title="$t('__workflowAction', { action: $t(StateConstant.ActionType.START_WORKFLOW_EXECUTION.i18nTitle) })"
  ...
  >
    <template #config-fields>
      ...
      <StateInputGroup
        ...
        :default-value="StartWorkflowExecutionConstant.ActionExecutionParams.INPUT"
        ...
        <AppFormFieldExpansionPanel :title="$t('__titleExecutionSettings')">
          ...
          <template v-if="!state.abortOnError">
            <StateInputGroup
              ...
              :default-value="new StartWorkflowExecutionDefaultOutput().removeActionType()"
              :expected-value-types="[JsonSchemaConstant.DataType.OBJECT.value]"
              :label="$t('__fieldDefaultOutput')"
              :tooltip="$t('__tooltipWorkflowActionDefaultOutput')"
              :schema="StartWorkflowExecutionConstant.ActionExecutionParams.DEFAULT_OUTPUT.jsonSchema"
              :on-update="update"
              enable-json-input-switch
            >
