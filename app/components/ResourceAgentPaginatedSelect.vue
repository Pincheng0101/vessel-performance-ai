...
<template>
  <ResourcePaginatedSelect
    ...
    :field-name="$t('__fieldAgent')"
    :instruction="$t('__instructionResourceAgent')"
    :module="ResourceConstant.Type.AGENT.module"
    :title="$t('__fieldAgent', 2)"
    :headers="[
      { title: $t('__fieldName'), key: 'name', sortable: true, sortKey: 'agent_name', link: item => ({ href: `/${ResourceConstant.Type.AGENT.path}/${item.id}`, target: '_blank' }) },
