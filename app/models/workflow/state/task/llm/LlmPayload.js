import { LlmConstant, StateConstant } from '~/constants';
...
import LlmDefaultOutput from './LlmDefaultOutput';

/**
 * @import { LlmActionExecutionPayload } from '~/models/server/llm'
 */

class LlmPayload extends TaskPayload {
  actionType = StateConstant.ActionType.LLM.value;

  /**
   * @param {boolean} params.useExternalMemoryOutput
   * @param {LlmActionExecutionPayload} params.llm
   * @param {LlmDefaultOutput} params.defaultOutput
   * @param {Object} params
  ...
  constructor({
    ...
    super({
      defaultOutput: (() => {
        ...
        return new LlmDefaultOutput(defaultOutput);
      })(),
      stateMemoryOutputSelector,
      streamingConfig,
      useExternalMemoryOutput,
    ...
    this.llm = (() => {
      ...
      return LlmActionExecutionPayloadFactory.create({
        ...llm,
        guardrailVersion: llm.guardrailVersion ?? LlmConstant.ActionExecutionParams.GUARDRAILS.version,
      });
    })();
    this.fallbackLlms = (() => {
  ...
  /**
   * @param {LlmPayload} actionPayload
   */
