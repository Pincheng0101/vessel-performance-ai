...
state.hasPermission = await hasWritePermission(ResourceConstant.Type.TEMPLATE.value);
...
const updateResource = async (resource) => {
  const { error } = await server.template.update(resource);
  if (error.value) {
    return;
  }
...
<template>
  ...
  <template v-else-if="data">
    <ResourceTemplateForm
      ...
      :on-discard="() => navigateTo(resourceUtils.getUrl(ResourceConstant.Type.TEMPLATE.value, data.id))"
    />
  </template>
</template>
