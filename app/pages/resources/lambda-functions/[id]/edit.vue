...
state.hasPermission = await hasWritePermission(ResourceConstant.Type.LAMBDA_FUNCTION.value);
...
const updateResource = async (resource) => {
  const { error } = await server.lambdaFunction.update(resource);
  if (error.value) {
    return;
  }
...
<template>
  ...
  <template v-else-if="data">
    <ResourceLambdaFunctionForm
      ...
      :on-discard="() => navigateTo(resourceUtils.getUrl(ResourceConstant.Type.LAMBDA_FUNCTION.value, data.id))"
    />
  </template>
</template>
