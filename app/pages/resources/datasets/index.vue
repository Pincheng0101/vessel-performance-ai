<script setup>
import { ListConstant, ResourceConstant, StatusConstant } from '~/constants';

const server = useServer();
const { openInNewTab } = useNavigation();
const { isDefaultResource } = useResource();
const { category, page, perPage, sortField, sortOrder, filters, filterLogic, query, nextTokenMap, initUrlParams, updateFiltersByCategory, goToPreviousPage } = usePagination({
  type: ResourceConstant.Type.DATASET.value,
  filterField: ResourceConstant.Type.DATASET.favoriteFilterField,
});
const { createSignal } = useAbortController();
const snackbarStore = useSnackbarStore();

const state = reactive({
  datasets: [],
  isLoading: false,
});

const fetchDatasets = async (pageValue = ListConstant.DefaultParams.PAGE) => {
  const signal = createSignal();

  state.isLoading = true;
  state.datasets = [];
  updateFiltersByCategory();
  const nextToken = pageValue === 1 ? null : nextTokenMap.value[pageValue - 1];
  const { data, error } = await server.dataset.list({
    nextToken,
    limit: perPage.value,
    sortField: sortField.value,
    sortOrder: sortOrder.value,
    filters: filters.value,
    filterLogic: filterLogic.value,
    query: query.value,
  }, {
    lazy: false,
    signal,
  });
  if (signal.aborted) return;

  if (error.value) {
    state.isLoading = false;
    return;
  }
  nextTokenMap.value[pageValue] = data.value.nextToken;
  state.datasets = data.value.data;
  page.value = pageValue;
  state.isLoading = false;
  // Navigate to the previous page if the current page has no items
  if (state.datasets.length === 0 && pageValue > 1) {
    goToPreviousPage();
    fetchDatasets(page.value);
  }
};

initUrlParams();
fetchDatasets(page.value);

const handleDelete = async ({ datasetId }) => {
  const { error } = await server.dataset.destroy({ datasetId });
  if (error.value) {
    return;
  }
  snackbarStore.setActionSuccess('__actionDelete');
  fetchDatasets(page.value);
};

const handlePageChange = (pageValue, pageToken) => {
  if (pageToken) {
    nextTokenMap.value[pageValue - 1] = pageToken;
  }
  fetchDatasets(pageValue);
};

const handlePerPageChange = (value) => {
  perPage.value = value;
  fetchDatasets();
};

const handleSortByChange = ({ key, order }) => {
  sortField.value = key;
  sortOrder.value = order;
  fetchDatasets();
};

const handleDuplicate = async ({ item, formData }) => {
  const { data, error } = await server.dataset.duplicate({
    datasetId: item.id,
    newDatasetName: formData.name,
  });
  if (error.value) {
    return;
  }
  snackbarStore.setActionSuccess('__actionDuplicate');
  navigateTo(resourceUtils.getUrl(ResourceConstant.Type.DATASET.value, data.value.id));
};

const handleQueryChange = (value) => {
  query.value = value;
  fetchDatasets();
};

const handleFiltersChange = (filtersValue, queryValue) => {
  filters.value = filtersValue;
  query.value = queryValue;
  fetchDatasets();
};

const handleCategoryChange = (value) => {
  category.value = value;
  query.value = ListConstant.DefaultParams.QUERY;
  updateFiltersByCategory({ reset: value === ListConstant.Category.ALL.value });
  fetchDatasets();
};
</script>

<template>
  <AppTable
    :title="$t('__fieldDataset', 2)"
    :icon="ResourceConstant.Type.DATASET.icon"
    :headers="[
      { title: $t('__fieldName'), key: 'name', sortable: true, sortKey: 'dataset_name', link: item => ({ href: `${resourceUtils.getUrl(ResourceConstant.Type.DATASET.value, item.id)}/evaluate` }) },
      { title: $t('__fieldId'), key: 'id' },
      { title: $t('__fieldStatus'), key: 'status', isStatus: true },
      { title: $t('__fieldLastUpdated'), key: 'updatedTs', isTimestamp: true, sortable: true },
    ]"
    :items="state.isLoading ? [] : state.datasets"
    :loading="state.isLoading"
    :has-next-page="!!nextTokenMap[page]"
    :category="category"
    :page="page"
    :sort-field="sortField"
    :sort-order="sortOrder"
    :per-page="perPage"
    :query="query"
    :next-token-map="nextTokenMap"
    :filters="filters"
    :filter-options="[
      { title: $t('__fieldName'), field: 'dataset_name' },
      { title: $t('__fieldId'), field: 'dataset_id' },
      { title: $t('__fieldStatus'), field: 'status', values: Object.values(StatusConstant.Resource).map(item => ({ ...item, title: $t(item.i18nTitle) })) },
    ]"
    :on-page-change="handlePageChange"
    :on-per-page-change="handlePerPageChange"
    :on-sort-by-change="handleSortByChange"
    :on-query-change="handleQueryChange"
    :on-filters-change="handleFiltersChange"
    :on-category-change="handleCategoryChange"
    enable-add-to-favorite
  >
    <template #header-actions>
      <AppIconButton
        icon="mdi-refresh"
        variant="text"
        :tooltip="$t('__actionRefresh')"
        @click="fetchDatasets(page)"
      />
      <AppIconButton
        icon="mdi-plus"
        class="primary-gradient"
        :tooltip="$t('__titleModifyItem', { action: $t('__actionCreate'), item: $t('__fieldStorage') })"
        :on-click="() => navigateTo(`${resourceUtils.getUrl(ResourceConstant.Type.DATASET.value)}/create`)"
      />
    </template>
    <template #row-menu="{ item, isHovering }">
      <template v-if="isHovering">
        <AppButton
          :height="32"
          :text="$t('__actionEvaluate')"
          append-icon="mdi-open-in-new"
          variant="flat"
          class="primary-gradient"
          @click="openInNewTab(`${resourceUtils.getUrl(ResourceConstant.Type.DATASET.value, item.id)}/evaluate`)"
        />
      </template>
      <template v-if="!isDefaultResource(item)">
        <ResourceActionMenu
          :item="item"
          :module="ResourceConstant.Type.DATASET.module"
          :persistent="isHovering"
          :item-label="$t('__fieldStorage')"
          :allow-delete-recursively="ResourceConstant.Type.DATASET.allowDeleteRecursively"
          :on-delete="handleDelete"
          :on-duplicate="handleDuplicate"
          :on-edit="item => navigateTo(`${resourceUtils.getUrl(ResourceConstant.Type.DATASET.value, item.id)}/edit`)"
          :on-resources-fetch="() => fetchDatasets(page)"
        />
      </template>
    </template>
    <template
      v-if="filters === ListConstant.DefaultParams.FILTERS"
      #no-data
    >
      <ResourceInitCard
        :icon="ResourceConstant.Type.DATASET.icon"
        :resource-label="$t('__fieldDataset')"
        :instruction="$t('__instructionResourceDataset')"
        :on-click="() => navigateTo(`${resourceUtils.getUrl(ResourceConstant.Type.DATASET.value)}/create`)"
      />
    </template>
  </AppTable>
</template>
