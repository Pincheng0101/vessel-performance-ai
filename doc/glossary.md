...
> **哪些欄位是合成的**見 [`synthetic-dataset.md`](synthetic-dataset.md)。
>
> **ISO 標準解說**：速度功率修正見 [`iso-15016.md`](iso-15016.md)、船體螺槳性能見
> [`iso-19030.md`](iso-19030.md)、**本專案的合規宣告與聲明偏差**見
> [`iso-19030-conformance.md`](iso-19030-conformance.md)。**維修時機最佳化**見
> [`maintenance-optimization.md`](maintenance-optimization.md)。**船舶諸元反推**見
> [`vessel.md`](vessel.md) 與 [`vessel_particulars.md`](vessel_particulars.md)。**查詢 API**見
> [`api.md`](api.md)。
>
> **本地 ML 預測軌 (local ML prediction track)**：特徵工程見
> [`feature-engineering.md`](feature-engineering.md)、模型分數見 [`model_summary.md`](model_summary.md)、
> 逐欄資料字典見 [`doc/ml_york/`](ml_york/)。
...
15. [附錄：本專案不存在的術語](#15-附錄本專案不存在的術語)
...
### ISO 19030 預設法 (Part 2) vs 替代法 (Part 3) 與聲明偏差 (Declared Deviation)

ISO 19030 **Part 2 是預設法 (default method)**，要求連續高頻取樣與區塊平均；**Part 3 允許替代法
(alternative method)**，前提是符合 Part 1 原則並**逐項明文聲明 (declared)** 每一處偏離。本專案的輸入是
**每日午報（每船每日一筆）**，無從高頻取樣，故實作的是 **ISO 19030-3 替代法**，並於
[`iso-19030-conformance.md`](iso-19030-conformance.md) 逐條聲明（`iso-19030-conformance.md:11`）。

> ⚠️ **這不是抄捷徑，是資料所逼。** 主要聲明偏差含：午報粒度取代高頻區塊、以閘門取代風修正、無舵角濾除、
> **無縱傾濾除且參考曲線不含縱傾項**（見〈縱傾〉）、參考曲線由在役資料而非試航擬合（見〈試航〉）。中位截距
> 使**約半數乾淨船體日的速度損失為負**——負值是擬合的性質，不是異常。

### 試航 / 速度試航 (Sea Trial / Speed Trial)

新船交船前在標準條件下量測速度–功率關係、經 ISO 15016 修正到平靜水面的**契約級基準**，是 ISO 19030 期望
的參考曲線來源（另可用模型試驗或 CFD）。**本專案沒有試航資料**，改以船隊**自身的在役午報**擬合乾淨船體
曲線——取每次船體清潔後 60 天內「假設乾淨」的視窗（`iso-19030-conformance.md:150`）。後果：S21/S22 等乾淨
點過少的船，其速度損失帶**未知正負號的常數偏移**，趨勢仍可信、絕對水準不可信。
...
### 能效設計指標 (Energy Efficiency Design Index, EEDI)

船舶**設計階段**的碳效率指標 (gCO₂/t·nm)，對**新造船 (newbuild)** 規範最低能效門檻——是 CII（營運達成
值）與 EEOI（營運運輸效率）的**設計面對應物**。它在試航時驗證，用的正是 ISO 15016 修正後的速度–功率曲線
（`iso-15016.md:36`）。**本專案不計算 EEDI**——船隊已在營運、無設計工況資料；此處僅為釐清它與 CII/EEOI
的分工而列。
...
### 新巴拿馬極限型 (New Panamax) / 陽明 W 級 (W-class)

反推鎖定的船型：**陽明 W 級 14,000 TEU New Panamax 貨櫃船**（LOA ~368 m、LBP 352 m、船寬 51 m、設計吃水
14.5 m），15 艘**共用同一船殼與主機**，W1/W2 僅差在螺距（見〈螺槳型式〉）。船型由午報反推鎖定後，與 MAN
*Propulsion of 14,000 teu container vessels* 設計基準船交叉驗證一致（`vessel_particulars.md`）。⚠️
**LOA/LBP/船寬/GT 等純幾何欄位無法從午報反推，皆由船級類比取得。**
...
### 結構吃水 / 結構排水量 (Scantling Draft / Displacement)

**結構吃水 15.8 m、結構排水量 185,900 t**（設計為 14.5 m / 166,500 t；`vessel_particulars.md`）——船體結構
容許的最大吃水。它是**離群裁剪的上界**：`clip_row` 把吃水裁到 `scantling_draft_m × 1.15`、排水量裁到
`displacement_scantling_t × 1.15`（`clean.py:77,85-87`，`_SCANTLING_HEADROOM = 1.15`）——超過結構極限 15%
的讀數在物理上不可能，逐格歸 null。
...
### 選定最大持續輸出 (Selected MCR, SMCR) 與海域裕度 (Sea Margin)

反推得 **SMCR ≈ 47,700 kW @ ~76 rpm**（`vessel_particulars.md`）。推導鏈：乾淨水域淨船殼曲線在設計船速
21.5 kn 給 `P_calm ≈ 35,260 kW`，加 **15% 海域裕度 (sea margin)** 得 NCR ≈ 40,550 kW，再由 `NCR = 0.85 ×
SMCR` 得 SMCR ≈ 47,700 kW；三項獨立證據（MAN 基準船、SFOC 拋物線頂點、實測出力 p99.9 = 82% SMCR）交叉
驗證。**海域裕度**是設計時為風浪預留的功率餘裕——它把「設計船速」定義在 NCR 而非 SMCR 上。
...
### 水線面係數 (Waterplane Coefficient, Cw)

水線面積相對其外接矩形的比值，**Cw ≈ 0.756**（`vessel_particulars.md`）。它是**船型反推的自洽性檢驗**：
貨櫃船慣有 `Cw ≈ Cb + 0.11`，本船隊 `Cw − Cb = 0.756 − 0.640 = 0.116`，據此反過來確認 L、B 選對。
...
`trim = fore_draft − after_draft` (m)，船艏艉吃水差（正值為艏傾）。兩個吃水欄的填充率達 99.95%，故縱傾
**算得出來**——但本專案**既不拿它濾點、也不放進參考曲線**（`iso-19030-conformance.md:137`）：

- **參考曲線只含排水量項、沒有縱傾項**（displacement-only），故沒有可對照的縱傾範圍可過濾；
- 憑觀測分位數硬定一條縱傾帶，是「看似嚴謹、實則無物理依據」的門檻。

⚠️ **後果：縱傾是每一個速度損失數字裡一個未濾除、未量化的誤差項。**
...
### 螺旋槳速度 (Propeller Speed, propeller_speed) ⚠️ 陷阱欄位

⚠️ **欄名/註解標「RPM」，實際是螺旋槳的理論前進速度 (theoretical advance speed)，單位「節 (kn)」。**
`schema.py:46` 的行內註解 `# measured: RPM` 是**沿用來源 README 的誤標**；程式碼的**實際行為**才是權威
（本檔第一原則），三項證據都指向「節」：

- 中位數 **16.3 kn**，與 STW（15.2 kn）同量級，而 `me_avg_rpm` 中位數是 52——若真是轉速不會落在此帶。
- 螺距即由它反解：`pitch_m = propeller_speed × 1852 / (60 × me_avg_rpm)`（`schema.py:115`），得 W1 = 9.886 m /
  W2 = 9.556 m，乾淨可信。
- 滑失恆等式 `(propeller_speed − STW) / propeller_speed` 重現午報自帶的 `me_slip`（中位絕對誤差 0.017%）——
  **只有在「節」這個讀法下恆等式才閉合。**

詳見 [`vessel.md`](vessel.md) / [`vessel_particulars.md`](vessel_particulars.md)。注意滑失計算餵入
`theoretical_speed_kn` 的 rpm 參數用的是 **`me_avg_rpm`**、不是本欄（`daily.py:167`）。

### 推力 / 推力商 (Thrust / Thrust Quotient, thrust / thrust_quotient) ⚠️

兩者皆為 **H 類（主機性能）欄位**，在遮蔽預測視窗中是 **HIDDEN**——屬 13 個 `leakage_columns` 之一，
**絕不可作為預測模型輸入**（見〈欄位分類 A/H/T/F〉）。且不含新資訊：

- **`thrust_quotient` 是衍生欄位**：`thrust_quotient = 1000 × thrust / horse_power`（與 `thrust/horse_power`
  相關係數 **+1.000**、比值 999.9997；`vessel_particulars.md`）——完全共線，沒有任何額外的螺旋槳幾何資訊。
- `thrust` 滿足 `T·V/P ≈ 0.82`，同樣推不出螺旋槳直徑。

精煉區只把兩欄**離群裁剪後留存**（`clean.py:33`），**沒有任何下游物理計算使用它們。**
...
### 低負荷調校 (Low-load Tuning)

此級主機採**低負荷調校**：SFOC（比油耗）最低點不在滿載，而在 **~60–65% 負荷**。本船隊實測 `SFOC` 對
`horse_power` 的拋物線頂點落在 **29,800 kW ≈ 63% SMCR**（`vessel_particulars.md`），與 MAN 設計慣例一致，
也解釋了為何操作面的最佳負載點取 **80% MCR**（見〈超額油費歸因〉的 operational 分量）而非滿載。

### 主機負載 (Engine Load, LOAD_PCT) ⚠️ 壞欄位

⚠️ **欄名標「%MCR」，卻與實際軸功率幾乎不相關**——不管主機出力 5 MW 還是 37 MW，`load_pct` 中位數都卡在
**~47%**（相關係數僅約 0.1–0.2；`vessel_particulars.md`）。**它不是 HP/MCR，不能拿來反推 MCR。**

- 它是 **H 類（主機性能）洩漏欄位**，遮蔽視窗中為 HIDDEN，不可作模型輸入（見〈欄位分類 A/H/T/F〉）。
- 精煉區只把它離群裁剪後留存（`clean.py:82`）。**操作面的負載罰則刻意不用它**——改由
  `load = horse_power / mcr_kw` 重算（`daily.py:289`），正因為 `load_pct` 不可信。
...
> **本層——精煉資料湖的分析層 (curated data-lake analytics layer)——全部是規則 + 穩健統計**
> （Theil-Sen、穩健 z 分數、異常偵測），**不使用機器學習，也不使用 SageMaker**（`grep -rin
> sagemaker` 於專案原始碼 = 0）。
>
> ⚠️ 但本專案另有一條**獨立的本地 ML 預測軌 (separate local ML prediction track)**：`scripts/` 與
> `ym_datalake/ml_york/` 以 scikit-learn / XGBoost 訓練 LinearRegression / RandomForest / XGBoost，
> 落地為 `models/*.pkl`，用於遮蔽視窗的燃油預測。見 [`feature-engineering.md`](feature-engineering.md)、
> [`model_summary.md`](model_summary.md)、[`doc/ml_york/`](ml_york/)。**「不使用機器學習」只適用於精煉區的
> 指標計算，不適用於這條預測軌。**
...
### 推論層 (Inference Layer) 與信心水準 (Confidence Level)

ISO 19030 只量**船體＋螺槳的合併效應**，本身**不提供拆分方法**。把速度損失歸因到「船體 vs 螺槳」是**疊在
ISO 之上的推論層 (inference layer)**，不是 ISO 輸出——儀表板明確標為「推論層」、視覺上與 ISO 速度損失圖區
隔，且**每格帶一個信心水準 (confidence level)**：由樣本數、距上次維修重置的天數、以及**是否有物理上獨立
的訊號佐證**驅動（`iso-19030-conformance.md:177`）。

> ⚠️ **循環佐證陷阱**：`excess_foc_mt` / `excess_cost_fouling_usd` 與 `uwi.hull_fouling_rating` 都是**由**
> 速度損失導出的，故永遠無法「佐證」速度損失——拿它們去對照，等於拿速度損失對照它自己。信心模型內建這條：
> 它們誰也不是佐證者，永遠到不了高信心。

### 船體＋螺槳合併效應 (Combined Hull + Propeller Effect)

`speed_loss_pct` 是**船體與螺槳合起來**的衰退，**從來不是純船體數字**（`iso-19030-conformance.md:179`）。
因此儀表板上該指標與超額油費的汙損分量都寫作「**船體＋螺槳**」而非「船體」。引用時不要把它講成「船體汙損」
——那是把一個合併量單方面歸給船體。

### EOQ / 年齡替換模型 (EOQ / Age-replacement Model)

最佳清潔時機 `T* = √(2K/β)`（見〈最佳清潔時機〉）不是憑空的公式，而是經典的 **EOQ（經濟訂購量）/ 年齡替換
(age-replacement)** 形狀：**一筆固定成本 K 攤提在一條線性成長的運行成本 β·t 上**，兩者的平衡點即 `T*`
（`maintenance-optimization.md:52`）。同一形狀也支撐併窗損益兩平（見〈併窗損益兩平〉）。
...
### 來源標記三分法 (Provenance Tags: measured / class / estimated)

`schema.py:17-24` 為每個欄位標一個來源標記，決定它能否被當作事實引用：

| 標記 | 意義 |
|---|---|
| `measured` | 讀自來源資料（可由 vt_fd / vessel / maintenance 重現） |
| `class` | W1/W2 姊妹船級的設計值，全級相同 |
| `estimated` | 合成的假設值，**永不可當作事實引用** |

`estimated` 集合為：日曆紀元、全部地理、全部 USD、UWI 數值訊號、事件成本/停役/地點。

### 欄位分類 A / H / T / F (Column Class)

來源 `dataset/README.md` 依欄位在**遮蔽預測視窗**內是否可見分四類（整理於 [`doc/ml_york/`](ml_york/)）——
它定義了「哪些欄位被藏起來要預測」：

| 類 | 內容 | 遮蔽視窗內 |
|---|---|---|
| **A** | 環境 / 航行 | 可見 |
| **H** | 主機性能（功率、負載、SFOC、滑失、推力） | **HIDDEN** |
| **T** | 油耗（總計、主機、各燃料別） | **HIDDEN**，標的格上為 **PREDICT** |
| **F** | 預測日篩選 / 建模輔助（`wind_scale`、`hours_full_speed`） | 可見 |

`leakage_columns`（13 個 = 6 個 H + 7 個 T）以標籤形式存在於 CSV，但**絕不可進入模型**——真實預測區它們是
HIDDEN。

### HIDDEN 與 PREDICT 儲存格 (HIDDEN vs PREDICT Cells)

`masked_flag` / `predict_fuel_type` 背後的兩種原始標記（`etl/source.py`）：

- **`HIDDEN`：4,429 格**（`dataset.md:127`）——值存在但被扣住，數值轉型後成 null。
- **`PREDICT`：102 格**——待預測的燃油標的格（14 個事件 × 每個 5–10 天）。

兩者在數值轉型**之前**先從原始字串解析（`PREDICT`→`is_predict`、`HIDDEN`/`PREDICT`→`is_masked`），故載入器
標記**保住了 null 化本會摧毀的資訊**。
...
### 剔除原因 (Reject Reason, reject_reason)

`valid_flag` 的**具名版對照物**：`filters.reject_reason(row, vessel)` 回傳該船日**卡在哪一道閘門**
（`filters.py:75`），通過則回 `None`（等同 `valid_flag = true`）。取值如 `masked`、`missing_propulsion`、
以及海軍係數/風級/速度/排水量/水深各閘的具名原因。**它讓「這天為什麼被剔除」可逐列稽核**，而不只是一個
真假旗標。
...
本資料湖的設計原則：**任何 KPI 都能往下鑽到「哪些天被丟掉、為什麼」。** 由幾條機制共同支撐——來源逐字
保存（見上）、`reject_reason` 的具名閘門、`displacement_source` 標記量測 vs 回填、`fit_pool` /
`n_fit_points` 記錄曲線用了哪個池、以及維修原子化可逆。**指標不是黑箱**：每一個彙總數字都能還原成它背後
的原始列與被套用的規則。
...
把散落在各章的「本專案沒有這個」callout 收攏成一張對照表，供反向查核——**看到別的資料集或舊版慣用這些詞，
不要套進本專案。**

| 不存在的術語 | 為什麼 / 程式碼佐證 |
|---|---|
| **Rubert A–F 螺槳粗糙度量表** | 螺槳/塗層狀況是**真實三級制** Good/Fair/Poor，非 Rubert 量表（`raw/uwi.py:15`、`schema.py:185` 兩處明文否定） |
| **NSTM 汙損評級** | `hull_fouling_rating` 是 0–100，但**不是 NSTM 評級**，也沒有「≥60 就清潔」的規則——它只是速度損失的線性重述（見〈UWI 數值訊號〉） |
| **SageMaker / 受管 ML 訓練服務** | 精煉分析層不用任何受管 ML 服務（`grep -rin sagemaker` 於專案原始碼 = 0）。⚠️ **機器學習本身「存在」**——有一條獨立的本地 ML 預測軌（見〈§13 開頭〉、[`doc/ml_york/`](ml_york/)）；這裡「不存在」的**只是 SageMaker 這類受管服務** |
| **港到港單一航段 `leg`** | 「航次」是多月環線 (rotation)，沒有 `leg` 欄位（見〈航次〉） |
| **潛水員 / ROV 檢查之分 (diver / ROV)** | `inspection_type` 只有 `UWI`（在水中）與 `DD`（乾塢），沒有 diver/ROV 之分（見〈水下檢查〉） |
| **`weather` 異常成因** | 成因鏈第 2 條需 `wind_scale ≥ 5`，但異常只跑在 `valid_flag`（風級 ≤4）的列上，兩者不相交 → 在列舉型別裡但資料中**永遠 0 列**（見〈天氣成因（不可達）〉） |
| **資料產生器 / 地面實況 (generator / ground truth)** | 來源是真實去識別化營運資料，沒有植入真值可供比對（見〈§0〉） |
| **資料表分割 (partitions)** | 全湖約 4 MB，20 張表全部未分割，`ship_id` 是普通欄位而非分割鍵（見〈§0〉） |

### ISO 15016 只解說、未實作的術語

[`iso-15016.md`](iso-15016.md) 完整解釋 ISO 15016，但**明文聲明「本文件解釋標準，不描述本程式碼做了什麼」**
（`iso-15016.md:7`）。以下試航級修正在本專案**未實作**（輸入是午報而非試航，缺對應資料）：

- **往返 / 對開航次 (double / reciprocal runs)**——午報沒有對開航段（`iso-15016.md:65`）。
- **「平均之平均」海流修正 (mean of means)**——需對開航次配對才可用（`iso-15016.md:66`）。
- **STAWAVE-2（頻譜、方向性浪阻）**——本專案只用 STAWAVE-1 迎浪近似（`iso-15016.md:68`，見〈波浪附加阻力〉）。
- **Lackenby 淺水修正**——本專案改以水深閘門處理淺水（`iso-15016.md:70`，見〈深水門檻〉）。
