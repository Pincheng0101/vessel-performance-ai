...
const fillForm = async (page) => {
  ...
  await page.getByLabel('Text Action', { exact: true }).click();
  // Wait for the form to be visible
  await page.locator('form').waitFor({ state: 'visible' });
  // Fill the form
  await page.getByLabel('Name', { exact: true }).fill('test_action');
  {
    const element = page.getByLabel('Template', { exact: true });
    await element.clear();
    await element.fill('test_template');
  }
  {
    const element = page.getByLabel('Template Variables Input Group', { exact: true });
    await element.hover();
    await element.getByLabel('Use JSON', { exact: true }).click();
  }
  {
    const element = page.getByLabel('Template Variables', { exact: true });
    await element.clear();
    await element.fill(JSON.stringify({ test: 'test_variable' }));
  }
  ...
  await page.getByLabel('Upload Output to External Memory', { exact: true }).check();
  {
    ...
    await element.clear();
    await element.fill(JSON.stringify({ text: '$.text' }));
  }
  ...
  }
  {
    ...
    await element.clear();
    await element.fill(JSON.stringify({ text: 'test_default_output' }));
  }
};
