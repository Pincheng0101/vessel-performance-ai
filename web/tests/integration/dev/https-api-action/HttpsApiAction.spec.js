...
const fillForm = async (page) => {
  ...
  await page.getByLabel('HTTPS API Action', { exact: true }).click();
  ...
  await page.getByLabel('URL', { exact: true }).fill('https://postman-echo.com/post');
  {
  ...
  }
  {
    ...
    await element.clear();
    await element.fill(JSON.stringify({ test: 'test_parameters' }));
  }
  {
    const element = page.getByLabel('Body Input Group', { exact: true });
    await element.hover();
    await element.getByLabel('Use JSON', { exact: true }).click();
  }
  {
    ...
    await element.clear();
    await element.fill(JSON.stringify({ data: { message: 'hello world' } }));
  }
...
test.describe('HttpsApiAction', () => {
  ...
  test('should produce the correct output', async () => {
    ...
    delete actualObject.output.headers;
    delete expectedObject.output.headers;
