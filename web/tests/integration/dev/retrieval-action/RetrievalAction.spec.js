...
const fillForm = async (page) => {
  ...
  await page.getByLabel('Retrieval Action', { exact: true }).click();
  // Wait for the form to be visible
  await page.locator('form').waitFor({ state: 'visible' });
  // Fill the form
  await page.getByLabel('Name', { exact: true }).fill('test_action');
  await page.getByLabel('Retrievers', { exact: true }).click();
  ...
  await page.getByLabel('Dialog').getByLabel('Advanced Settings', { exact: true }).click();
  {
  ...
  await page.getByLabel('Search', { exact: true }).fill('e2e_testing_ranker_cohere_for_retriever_and_retrieval_action');
  await page.getByLabel('e2e_testing_ranker_cohere_for_retriever_and_retrieval_action', { exact: true }).click();
  await page.getByLabel('Resource Dialog', { exact: true }).getByLabel('Save', { exact: true }).click();
  await page.getByLabel('Query Source', { exact: true }).click();
  await page.getByLabel('Custom Query', { exact: true }).click();
  {
    const element = page.getByLabel('Query String', { exact: true });
    await element.clear();
    await element.fill('Which fruit has the most vitamin C?');
  }
  await page.getByLabel('Ranking Fields', { exact: true }).fill('retriever_chunk');
  await page.getByLabel('Document Limit', { exact: true }).fill('15');
