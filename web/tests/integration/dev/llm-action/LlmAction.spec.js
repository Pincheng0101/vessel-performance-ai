...
const fillForm = async (page) => {
  ...
  await page.getByLabel('LLM Action', { exact: true }).click();
  ...
  }
  {
    ...
    await element.clear();
    await element.fill(JSON.stringify({ message: 'test_default_output_message' }));
  }
};
...
test.describe('LlmAction', () => {
  ...
  test('should produce the correct output', async () => {
    ...
    delete actualObject.message;
    delete expectedObject.source_llm_id;
    delete expectedObject.message;
