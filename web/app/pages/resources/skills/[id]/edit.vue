...
state.hasPermission = await hasWritePermission(ResourceConstant.Type.SKILL.value);
...
const updateResource = async (resource) => {
  const { error } = await server.skill.update(resource);
  if (error.value) {
    return;
  }
...
<template>
  ...
  <template v-else-if="data">
    <ResourceSkillForm
      ...
      :on-discard="() => navigateTo(resourceUtils.getUrl(ResourceConstant.Type.SKILL.value, data.id))"
    />
  </template>
</template>
