<script setup>
import { ListConstant, LlmConstant, ResourceConstant, StatusConstant } from '~/constants';

const server = useServer();
const { category, page, perPage, sortField, sortOrder, filters, filterLogic, query, nextTokenMap, initUrlParams, updateFiltersByCategory, goToPreviousPage } = usePagination({
  type: ResourceConstant.Type.LLM.value,
  filterField: ResourceConstant.Type.LLM.favoriteFilterField,
});
const { createSignal } = useAbortController();
const snackbarStore = useSnackbarStore();

const state = reactive({
  llms: [],
  isLoading: false,
});

const fetchLlms = async (pageValue = ListConstant.DefaultParams.PAGE) => {
  const signal = createSignal();

  state.isLoading = true;
  state.llms = [];
  updateFiltersByCategory();
  const nextToken = pageValue === 1 ? null : nextTokenMap.value[pageValue - 1];
  const { data, error } = await server.llm.list({
    nextToken,
    limit: perPage.value,
    sortField: sortField.value,
    sortOrder: sortOrder.value,
    filters: filters.value,
    filterLogic: filterLogic.value,
    query: query.value,
  }, {
    lazy: false,
    signal,
  });
  if (signal.aborted) return;

  if (error.value) {
    state.isLoading = false;
    return;
  }
  nextTokenMap.value[pageValue] = data.value.nextToken;
  state.llms = data.value.data;
  page.value = pageValue;
  state.isLoading = false;
  // Navigate to the previous page if the current page has no items
  if (state.llms.length === 0 && pageValue > 1) {
    goToPreviousPage();
    fetchLlms(page.value);
  }
};

initUrlParams();
fetchLlms(page.value);

const handleDelete = async ({ llmId }) => {
  const { error } = await server.llm.destroy({ llmId });
  if (error.value) {
    return;
  }
  snackbarStore.setActionSuccess('__actionDelete');
  fetchLlms(page.value);
};

const handlePageChange = (pageValue, pageToken) => {
  if (pageToken) {
    nextTokenMap.value[pageValue - 1] = pageToken;
  }
  fetchLlms(pageValue);
};

const handlePerPageChange = (value) => {
  perPage.value = value;
  fetchLlms();
};

const handleSortByChange = ({ key, order }) => {
  sortField.value = key;
  sortOrder.value = order;
  fetchLlms();
};

const handleDuplicate = async ({ item, formData }) => {
  const { data, error } = await server.llm.duplicate({
    llmId: item.id,
    newLlmName: formData.name,
  });
  if (error.value) {
    return;
  }
  snackbarStore.setActionSuccess('__actionDuplicate');
  navigateTo(resourceUtils.getUrl(ResourceConstant.Type.LLM.value, data.value.id));
};

const handleQueryChange = (value) => {
  query.value = value;
  fetchLlms();
};

const handleFiltersChange = (filtersValue, queryValue) => {
  filters.value = filtersValue;
  query.value = queryValue;
  fetchLlms();
};

const handleCategoryChange = (value) => {
  category.value = value;
  query.value = ListConstant.DefaultParams.QUERY;
  updateFiltersByCategory({ reset: value === ListConstant.Category.ALL.value });
  fetchLlms();
};
</script>

<template>
  <AppTable
    :title="$t('__fieldLlm', 2)"
    :icon="ResourceConstant.Type.LLM.icon"
    :headers="[
      { title: $t('__fieldName'), key: 'name', sortable: true, sortKey: 'llm_name', link: item => ({ href: resourceUtils.getUrl(ResourceConstant.Type.LLM.value, item.id) }) },
      { title: $t('__fieldId'), key: 'id' },
      { title: $t('__fieldType'), key: 'type', value: item => findField(LlmConstant.Type, item.type, 'title'), iconPath: item => findField(LlmConstant.Type, item.type, 'iconPath') },
      { title: $t('__fieldModel'), key: 'model', value: item => findField(LlmConstant.Model, item.model, 'title') || item.model }, // Fallback for unknown model
      { title: $t('__fieldStatus'), key: 'status', isStatus: true },
      { title: $t('__fieldLastUpdated'), key: 'updatedTs', isTimestamp: true, sortable: true },
    ]"
    :items="state.isLoading ? [] : state.llms"
    :loading="state.isLoading"
    :has-next-page="!!nextTokenMap[page]"
    :category="category"
    :page="page"
    :sort-field="sortField"
    :sort-order="sortOrder"
    :per-page="perPage"
    :query="query"
    :next-token-map="nextTokenMap"
    :filters="filters"
    :filter-options="[
      { title: $t('__fieldName'), field: 'llm_name' },
      { title: $t('__fieldId'), field: 'llm_id' },
      { title: $t('__fieldType'), field: 'llm_type', values: Object.values(LlmConstant.Type).map(item => ({ ...item, subtitle: $t(item.i18nSubtitle) })) },
      { title: $t('__fieldModel'), field: 'model', values: Object.values(LlmConstant.Model).map(item => ({ ...item, title: `${item.provider} - ${item.title}` })) },
      { title: $t('__fieldStatus'), field: 'status', values: Object.values(StatusConstant.Resource).map(item => ({ ...item, title: $t(item.i18nTitle) })) },
    ]"
    :on-page-change="handlePageChange"
    :on-per-page-change="handlePerPageChange"
    :on-sort-by-change="handleSortByChange"
    :on-query-change="handleQueryChange"
    :on-filters-change="handleFiltersChange"
    :on-category-change="handleCategoryChange"
    enable-add-to-favorite
  >
    <template #header-actions>
      <AppIconButton
        icon="mdi-refresh"
        variant="text"
        :tooltip="$t('__actionRefresh')"
        @click="fetchLlms(page)"
      />
      <AppIconButton
        icon="mdi-plus"
        class="primary-gradient"
        :tooltip="$t('__titleModifyItem', { action: $t('__actionCreate'), item: $t('__fieldLlm') })"
        :on-click="() => navigateTo(`${resourceUtils.getUrl(ResourceConstant.Type.LLM.value)}/create`)"
      />
    </template>
    <template #row-menu="{ item, isHovering }">
      <ResourceActionMenu
        :item="item"
        :module="ResourceConstant.Type.LLM.module"
        :persistent="isHovering"
        :item-label="$t('__fieldLlm')"
        :allow-delete-recursively="ResourceConstant.Type.LLM.allowDeleteRecursively"
        :on-delete="handleDelete"
        :on-duplicate="handleDuplicate"
        :on-edit="item => navigateTo(`${resourceUtils.getUrl(ResourceConstant.Type.LLM.value, item.id)}/edit`)"
        :on-resources-fetch="() => fetchLlms(page)"
      />
    </template>
    <template
      v-if="filters === ListConstant.DefaultParams.FILTERS"
      #no-data
    >
      <ResourceInitCard
        :icon="ResourceConstant.Type.LLM.icon"
        :resource-label="$t('__fieldLlm')"
        :instruction="$t('__instructionResourceLlm')"
        :on-click="() => navigateTo(`${resourceUtils.getUrl(ResourceConstant.Type.LLM.value)}/create`)"
      />
    </template>
  </AppTable>
</template>
