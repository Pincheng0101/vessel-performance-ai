...
import { BedrockNovaLlm } from '~/models/server/llm';

/**
 * @type {{ resource: BedrockNovaLlm }}
 */
...
const state = reactive({
  modelItems: Object.values(LlmConstant.BedrockNovaModel),
  regionItems: [],
...
const maxTokensParam = computed(() => findField(LlmConstant.BedrockNovaModel, model.value, 'maxTokens'));
const temperatureParam = computed(() => findField(LlmConstant.BedrockNovaModel, model.value, 'temperature'));
const topPParam = computed(() => findField(LlmConstant.BedrockNovaModel, model.value, 'topP'));
const topKParam = computed(() => findField(LlmConstant.BedrockNovaModel, model.value, 'topK'));
...
const handleModelChange = (v) => {
  state.regionItems = findField(LlmConstant.BedrockNovaModel, v, 'supportedRegions') || [];
  region.value = null;
...
const defaultParams = props.resource || new BedrockNovaLlm();
</script>
