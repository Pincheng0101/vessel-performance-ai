...
state.formData.trackToolResults ??= AgentConstant.ToolType.SEARCH_ENGINE.defaultTrackToolResults;
...
<template>
  <AppForm
    :form-title="$t('__titleModifyToolItem', { action: props.item ? $t('__actionEdit') : $t('__actionAdd'), item: $t('__fieldSearchEngine') })"
    :on-submit="submit"
  ...
  >
    <template #body>
      ...
      <ResourceSearchEnginePaginatedSelect
        v-model="state.formData.searchEngineId"
        :return-object="false"
        :not-found-object-id="props.notFoundResource?.type === ResourceConstant.Type.SEARCH_ENGINE.module ? props.notFoundResource.id : null"
        required
      />
      <AppInputGroup
        v-slot="{ id, label }"
