...
state.formData.trackToolResults ??= AgentConstant.ToolType.AGENT.defaultTrackToolResults;
...
<template>
  <AppForm
    :form-title="$t('__titleModifyToolItem', { action: props.item ? $t('__actionEdit') : $t('__actionAdd'), item: $t('__fieldAgent') })"
    :on-submit="submit"
  ...
  >
    <template #body>
      ...
      <ResourceAgentPaginatedSelect
        v-model="state.formData.agentId"
        :disable-condition="{
          conditions: [
            { field: 'agentId', operator: '==', value: route.params.id },
          ],
        }"
        :disabled-tooltip="$t('__tooltipAgentToolAgentCannotSelectSelf')"
        :return-object="false"
        :not-found-object-id="props.notFoundResource?.type === ResourceConstant.Type.AGENT.module ? props.notFoundResource.id : null"
        required
      />
      <AppInputGroup
        v-slot="{ id, label }"
