...
import { BedrockGptOssLlm } from '~/models/server/llm';

/**
 * @type {{ resource: BedrockGptOssLlm }}
 */
...
const state = reactive({
  modelItems: Object.values(LlmConstant.BedrockGptOssModel),
  regionItems: [],
...
const maxTokensParam = computed(() => findField(LlmConstant.BedrockGptOssModel, model.value, 'maxTokens'));
const temperatureParam = computed(() => findField(LlmConstant.BedrockGptOssModel, model.value, 'temperature'));
const topPParam = computed(() => findField(LlmConstant.BedrockGptOssModel, model.value, 'topP'));
...
const handleModelChange = (v) => {
  state.regionItems = findField(LlmConstant.BedrockGptOssModel, v, 'supportedRegions') || [];
  region.value = null;
...
const defaultParams = props.resource || new BedrockGptOssLlm();
</script>
