...
const updateDependencies = async () => {
  ...
  await fetchDependencies({
    ...
    resourceId: formData.value.agentId,
  });
...
<template>
  ...
  <template v-if="!props.resource && formData.agentId">
    <AppInputGroup
      bordered
    ...
    >
      <AppInputGroup
        ...
        :tooltip="$t('__tooltipGrantDependencyPermissions', {
          ...
          resourceType: $t('__fieldAgent').toLowerCase(),
      ...
      <template v-if="formData.grantDependencyPermissions">
        ...
        <template v-else>
          ...
          <template v-else>
            <p>{{ $t('__instructionNoDependency', { item: $t('__fieldAgent') }) }}</p>
          </template>
        </template>
      </template>
    </AppInputGroup>
