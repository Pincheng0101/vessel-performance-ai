...
const Term = Object.freeze({
  ...
  // Performance
  speedLoss: '相同主機功率下，實際船速較乾淨船體基準的下降百分比：(v_expected − STW) / v_expected × 100。正值＝速度損失＝性能衰退，這是 ISO 19030-2 帶號慣例的反號（該慣例以負值代表衰退），屬 ISO 19030-3 下的宣告性偏離。反映的是船體「與」螺槳的合併效應——ISO 19030 不提供兩者的拆分方法。此欄顯示的是該船最近 30 個通過 ISO 19030 篩選之有效日（valid day）的平均，而非單一日讀數：單日值受基準曲線的擬合散布支配，不足以判讀船體狀態。',
  ...
  excessCost: '每日超額燃油成本＝相對乾淨船體、平穩海況多耗的燃油 × 油價，拆解為三個來源並加總：船體＋螺槳汙損（最大且可經由清潔／拋光改善；此項由速度損失導出，ISO 19030 不拆分船體與螺槳，故兩者合計）、天氣／海況（季節性、多不可控）、操作（航速／裝載等）。KPI 為近 30 天平均的每日成本；圖為各來源每日金額的 30 天移動平均堆疊。營運總覽／船隊總覽的「Excess fuel cost」是全船隊「每艘船最新一天」數字的加總，口徑跟這裡的 30 天平均不同，數字無法直接比較。',
  ...
  speedLossDistribution: '全船隊各船速度損失的分布；每艘船以其最近 30 個通過 ISO 19030 篩選之有效日（valid day）的平均計入一格。「≤ 0%」代表該船處於其乾淨船體基準線（baseline）上，即無可量測的速度損失——基準曲線以中位數截距擬合，故乾淨船體本就有約一半的日子落在曲線之上。',
