"""RandomForest comparison model for the ME full-speed fuel-consumption benchmark.
...
``exp(pred) * hours_full_speed / fuel_lcv``. sklearn's RandomForest cannot ingest the NaN-bearing rolling
/ ``*_missing`` features, so it sits behind a median :class:`SimpleImputer` (the ``feature_engineering``
baseline pattern). Native importance is mean impurity decrease.
"""
...
def train_model(train_df: pd.DataFrame, features: list[str], seed: int = 42) -> Pipeline:
    """Fit median-impute -> RandomForest on the steady single-fuel labelled rows."""
