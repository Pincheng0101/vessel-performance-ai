...
const fillForm = async (page) => {
  ...
  await page.getByLabel('Ranker Action', { exact: true }).click();
  // Wait for the form to be visible
  await page.locator('form').waitFor({ state: 'visible' });
  // Fill the form
  await page.getByLabel('Name', { exact: true }).fill('test_action');
  await page.getByLabel('Ranker', { exact: true }).click();
  await page.getByLabel('Search', { exact: true }).fill('e2e_testing_ranker_jaccard_similarity');
  await page.getByLabel('e2e_testing_ranker_jaccard_similarity', { exact: true }).click();
  ...
  await page.getByLabel('Custom Query', { exact: true }).click();
  {
    ...
    await element.clear();
    await element.fill('vegan healthy no-meat tofu salad');
  }
  await page.getByLabel('Ranking Fields', { exact: true }).fill('ingredients');
  await page.getByLabel('ingredients', { exact: true }).click();
  {
    const element = page.getByLabel('Documents Input Group', { exact: true });
    await element.hover();
    await element.getByLabel('Use JSON', { exact: true }).click();
  }
  {
    const element = page.getByLabel('Documents', { exact: true });
    await element.clear();
    await element.fill(JSON.stringify([{ dish: 'Tofu Salad Bowl', ingredients: 'tofu, lettuce, sesame dressing, cucumber, healthy' }, { dish: 'Beef Noodle Soup', ingredients: 'beef, noodle, soy sauce, bok choy' }, { dish: 'Taiwanese Fried Chicken', ingredients: 'chicken, flour, pepper, garlic, crispy' }]));
  }
  ...
  await page.getByLabel('Relevance Threshold', { exact: true }).fill('0');
