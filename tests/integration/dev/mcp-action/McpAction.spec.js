...
const fillForm = async (page) => {
  ...
  await page.getByLabel('MCP Action', { exact: true }).click();
  // Wait for the form to be visible
  await page.locator('form').waitFor({ state: 'visible' });
  // Fill the form
  await page.getByLabel('Name', { exact: true }).fill('test_action');
  await page.getByLabel('MCP Server', { exact: true }).click();
  await page.getByLabel('Search', { exact: true }).fill('e2e_testing_mcp_server');
  await page.getByLabel('e2e_testing_mcp_server', { exact: true }).click();
  ...
  await page.getByLabel('Search', { exact: true }).fill('greeting');
  await page.getByLabel('greeting', { exact: true }).click();
  await page.getByLabel('MCP Server Tools Dialog', { exact: true }).getByLabel('Save', { exact: true }).click();
  {
  ...
  }
  {
    ...
    await element.clear();
    await element.fill(JSON.stringify({ name: 'Tester' }));
  }
